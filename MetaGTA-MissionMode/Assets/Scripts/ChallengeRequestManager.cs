...
public class ChallengeRequestManager : MonoBehaviour, IOnEventCallback
{
    ...
    public System.Action<int,int,int,int> OnPlayerAcceptedChallenge;
    ...
    void RequestFightRPC(string _uid, PhotonMessageInfo info)
    {
        ...
        if (pv.IsMine)
        {
            ...
            {
                if (MetaManager.Instance.inChallengePlayer != null || MissionSystem.Instance.isDoingMission) return;
    ...
    public void RequestFightAction(bool _action)
    {
        ...
        if (_action)
        {
            ...
            OnPlayerAcceptedChallenge?.Invoke(race_start_index,race_path_index,race_env_poz_index ,0);
        }

        /*RaceObjectPool.Instance.offsetsOfGeneration = offsets;*/
        SendFightAction(_action, race_start_index,race_path_index, race_env_poz_index, MetaManager.Instance.inChallengePlayer.UserId, PhotonNetwork.LocalPlayer.UserId);

    }

    private void SendFightAction(bool _action, int racePozIndex, int racePathIndex,int raceEnvPoz,  string _p1uid, string _p2uid)
    {
      
        object[] content = new object[] { _action, racePozIndex, racePathIndex, raceEnvPoz, _p1uid, _p2uid }; // Array contains the target position and the IDs of the selected units
                    ...
                    if (_action)
                    {
                        ...
                        if (pv.IsMine)
                        {
                           // can_move = false;
                            ...
                            OnPlayerAcceptedChallenge?.Invoke(racePozIndex,racePathIndex, raceEnvIndex, 1);
