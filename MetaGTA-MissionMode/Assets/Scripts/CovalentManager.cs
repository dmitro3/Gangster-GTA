...
public class CovalentManager : MonoBehaviour
{
                ...
                case UnityWebRequest.Result.Success:
                    ...
                    if (_data.GetField("data").HasField("items"))
                    {
                        ...
                        for (int i = 0; i < _data.GetField("data").GetField("items").list.Count; i++)
                        {
                            ...
                            if (BCCoreManager.contract.ToLower().Equals(_add))
                            {
    ...
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(GetMetaDataPreURL + BCCoreManager.contract + GetMetaDataMidURL + _tokenid + GetMetaDataPostURL))
        {
